...
给一个已经关闭channel发送数据，会引起死锁造成panic；  
...
map的底层是一个散列表，主要结构有两个，hmap和bmap，其中通过hash的低B位选择是哪个桶，高八位确定在桶中的哪个位置。  
哈希碰撞的解决办法：  
...
float32在强转成float64时会造成精度丢失问题，通常可以使用decimal包来解决

## 14. 单元测试

单元测试要求覆盖到主要的业务逻辑，遍历所有的错误返回，并且通过使用 gomonkey 打桩模拟不同返回下的业务处理。
使用goconvey测试框架

## 15. Copy函数

如果需要复制Slice，需要使用copy(dst, src)方法，需要注意copy实际复制的元素个数是两者的最小值。
